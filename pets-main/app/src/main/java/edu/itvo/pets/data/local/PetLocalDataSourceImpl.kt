...
class PetLocalDataSourceImpl @Inject constructor(private val petDao: PetDao): PetLocalDataSource {
    ...
    override suspend fun getDeletedPets(): Flow<PetResponse?> {
        val pets = petDao.getDeletedPets()
        val data = pets.map { it.toListPetModel() }
        val listPetModel = data.first()
        return flow { emit(PetResponse(true, "list deleted pets", listPetModel)) }
    }
    ...
    override suspend fun softDelete(petId: Int) {
        petDao.softDelete(petId)
    }

    override suspend fun restore(petId: Int) {
        petDao.restore(petId)
    }
}