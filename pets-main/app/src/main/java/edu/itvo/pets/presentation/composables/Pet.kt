...
fun Pet(
    ...
    context: Context,
    petId: Int? = null,  // Agregamos el parámetro petId como opcional
    onSaveSuccess: () -> Unit = {}
) {
        ...
        Column(
            ...
            Card(modifier = modifier.fillMaxWidth()) {
                Column(
                    ...
                    Button(onClick = {
                        if (petId != null) {
                            // Si hay un petId, actualizamos la mascota existente
                            viewModel.onEvent(
                            ...
                            // Si no hay petId, creamos una nueva mascota
                            viewModel.onEvent(
                                PetViewModel.PetEvent.AddClicked(
                                    name = state.name,
                                    description = state.description,
                        ...
                        Text(text = if (petId != null) "Actualizar" else "Guardar")
                    }
                    ...
                    if (hasSaved) {
                        AlertDialog(
                            onDismissRequest = {
                                hasSaved = false
                                viewModel.onEvent(PetViewModel.PetEvent.Reset)
                                onSaveSuccess()
                            },
                            confirmButton = {
                                Button(onClick = {
                            ...
                            title = { Text("Éxito") },
                            text = { Text(if (petId != null) "Mascota actualizada satisfactoriamente" else "Mascota guardada satisfactoriamente") }
                        )
                    }
                }
            }
