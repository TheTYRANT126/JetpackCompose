...
@Singleton
class PetRepositoryImpl @Inject constructor(private val localDataSource: PetLocalDataSource): PetRepository {
    ...
    override suspend fun softDeletePet(petId: Int) {
        localDataSource.softDelete(petId)
    }

    override suspend fun restorePet(petId: Int) {
        localDataSource.restore(petId)
    }

    override suspend fun getDeletedPets(): Flow<PetResponse?> {
        return localDataSource.getDeletedPets()
    }
}