...
interface PetDao{
    ...
    @Query("UPDATE pet SET isDeleted = 1 WHERE id = :petId")
    fun softDelete(petId: Int): Int

    @Query("UPDATE pet SET isDeleted = 0 WHERE id = :petId")
    fun restore(petId: Int): Int

    @Query("SELECT * FROM pet WHERE isDeleted = 1")
    fun getDeletedPets(): Flow<List<PetEntity>>
