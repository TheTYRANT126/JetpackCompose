...
class PetUseCase @Inject constructor(private val petRepository: PetRepository) {
    ...
    suspend fun softDelete(petId: Int) = petRepository.softDeletePet(petId)
    suspend fun restore(petId: Int) = petRepository.restorePet(petId)
    suspend fun getDeletedPets(): Flow<PetResponse?> = petRepository.getDeletedPets()
